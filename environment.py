...
class ReKepOGEnv:
    ...
    def save_video(self, save_path=None):
        ...
        for rgb in self.video_cache:
            ...
            # Ensure that rgb is now a numpy ndarray
            video_writer.append_data(rgb)
    ...
    def _check_reached_ee(self, target_pos, target_xyzw, pos_threshold, rot_threshold):
        ...
        current_pos_np = current_pos.detach().cpu().numpy() if isinstance(current_pos, torch.Tensor) else current_pos
        pos_diff = (target_pos - current_pos_np).flatten()
    ...
    def _move_to_waypoint(self, target_pose_world, pos_threshold=0.02, rot_threshold=3.0, max_steps=10):
        ...
        while count < max_steps:
            ...
            # 确保获取的相对位置是 NumPy 数组
            relative_eef_position = self.robot.get_relative_eef_position()
            relative_eef_position_np = relative_eef_position.detach().cpu().numpy() if isinstance(relative_eef_position, torch.Tensor) else relative_eef_position

            relative_position = target_pose_robot[:3, 3] - relative_eef_position_np
            relative_quat = T.quat_distance(T.mat2quat(target_pose_robot[:3, :3]), self.robot.get_relative_eef_orientation())
